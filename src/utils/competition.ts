 ...
 */
export function safeBigIntConversion(
  ...
  try {
    ...
    const cleanedValue = stringValue.replace(
      /^(-?)(.*)$/,
      (_, sign, nums) => sign + nums.replace(/[^\d]/g, "")
    );
...
export function analyzeResourceFocus(resourceBalances: Record<string, string>): any {
  try {
    ...
    if (balances["graphite"] > 0n && balances["graphene"] > 0n) {
      ratios["graphiteToGraphene"] =
        Number((balances["graphene"] * 100n) / balances["graphite"]) / 100;
    }

    // Neodymium to Dysprosium ratio
    if (balances["neodymium"] > 0n && balances["dysprosium"] > 0n) {
      ratios["neodymiumToDysprosium"] =
        Number((balances["dysprosium"] * 100n) / balances["neodymium"]) / 100;
    }

    // Dysprosium to Yttrium ratio
    if (balances["dysprosium"] > 0n && balances["yttrium"] > 0n) {
      ratios["dysprosiumToYttrium"] =
        Number((balances["yttrium"] * 100n) / balances["dysprosium"]) / 100;
    }
...
export function analyzePathPreference(
  ...
  try {
    ...
    if (totalResourceValue > 0n) {
      metrics.resources.graphene.percentage =
        Number((metrics.resources.graphene.raw * 10000n) / totalResourceValue) / 100;
      metrics.resources.yttrium.percentage =
        Number((metrics.resources.yttrium.raw * 10000n) / totalResourceValue) / 100;
    }
    ...
    if (positions.length > 0) {
      ...
      if (totalPositions > 0) {
        metrics.liquidity.graphene.percentage =
          (metrics.liquidity.graphene.count * 100) / totalPositions;
        metrics.liquidity.yttrium.percentage =
          (metrics.liquidity.yttrium.count * 100) / totalPositions;
    ...
    const sumOfAllActivities = metrics.overall.graphene.score + metrics.overall.yttrium.score;
...
export function analyzeLiquidityStrategy(liquidityPositions: any): any {
  try {
    ...
    const metrics = {
      ...
      byResourceName: {} as Record<
        string,
        { count: number; liquidity: bigint; percentage: number }
      >,
    ...
    const resourceTypeCount = Object.values(metrics.byResourceType).filter(
      (type) => type.count > 0
    ).length;

    const resourceNameCount = Object.values(metrics.byResourceName).filter(
      (resource) => resource.count > 0
    ).length;

    metrics.strategyIndicators.resourceDiversification = Math.min(
      100,
      (resourceTypeCount / 4) * 50 + (resourceNameCount / 7) * 50
    );

    // Advanced resource focus
    metrics.strategyIndicators.advancedResourceFocus =
      metrics.byResourceType.advanced.percentage + metrics.byResourceType.he3.percentage * 0.5;
...
export function analyzeStakingStrategy(stakePositions: any): any {
  try {
    ...
    const metrics = {
      ...
      byFarmId: {} as Record<
        string,
        { count: number; stakedAmount: bigint; rewards: bigint; percentage: number }
      >,
      byPath: {
    ...
    if (metrics.totalStaked > 0n) {
      ...
      const totalPathStaked =
        metrics.byPath.graphene.stakedAmount + metrics.byPath.yttrium.stakedAmount;
    ...
    const advancedWeight =
      (metrics.byFarmType.he3Production.percentage + metrics.byFarmType.he3Staking.percentage) *
      100;
    ...
    const distributionFactor =
      Object.values(metrics.byFarmType).filter((type) => type.count > 0).length * 12.5; // Max 50 if all 4 types are used

    metrics.strategyIndicators.yieldFarmingIntensity = Math.min(
      100,
      positionFactor + distributionFactor
    );
...
export function determineOverallStrategy(
  ...
  try {
    ...
    const pathAnalysis = analyzePathPreference(
      resourceBalances,
      liquidityPositions,
      stakePositions
    );
    ...
    if (resourceBalances) {
      // Check for intermediate resources
      const hasGraphite =
        resourceBalances["graphite"] !== undefined &&
        safeBigIntConversion(resourceBalances["graphite"]) > 0n;
      const hasDysprosium =
        resourceBalances["dysprosium"] !== undefined &&
      ...
      // Check for base resources
      const hasCarbon =
        resourceBalances["carbon"] !== undefined &&
        safeBigIntConversion(resourceBalances["carbon"]) > 0n;
      const hasNeodymium =
        resourceBalances["neodymium"] !== undefined &&
    ...
    if (resourceAnalysis && !resourceAnalysis.error && resourceAnalysis.pathTotals) {
      metrics.activityDistribution.resource.graphenePath =
        resourceAnalysis.pathTotals.graphenePath?.percentage || 0;
      metrics.activityDistribution.resource.yttriumPath =
        resourceAnalysis.pathTotals.yttriumPath?.percentage || 0;
      metrics.activityDistribution.resource.he3Direct =
        resourceAnalysis.pathTotals.helium3?.percentage || 0;
    }
    ...
    if (stakingAnalysis && !stakingAnalysis.error && stakingAnalysis.hasPositions) {
      ...
      metrics.activityDistribution.staking.graphenePath =
        stakingAnalysis.byPath?.graphene?.percentage || 0;
      metrics.activityDistribution.staking.yttriumPath =
        stakingAnalysis.byPath?.yttrium?.percentage || 0;
      metrics.activityDistribution.staking.he3Production =
        stakingAnalysis.byFarmType?.he3Production?.percentage || 0;
      metrics.activityDistribution.staking.he3Direct =
        stakingAnalysis.byFarmType?.he3Staking?.percentage || 0;
    }
    ...
    if (resourceAnalysis && !resourceAnalysis.error) {
      ...
      metrics.compositeMetrics.resourceOptimization = Math.min(
        100,
        dominantResourcePercentage * 1.5 - (resourceCount > 3 ? (resourceCount - 3) * 10 : 0)
      );
    }
    ...
    const grapheneIntegration =
      ...
      (resourceBalances["graphite"] && safeBigIntConversion(resourceBalances["graphite"]) > 0n
        ? 30
        : 0) +
      (resourceBalances["graphene"] && safeBigIntConversion(resourceBalances["graphene"]) > 0n
        ? 50
        : 0);

    const yttriumIntegration =
      (metrics.gamePhaseIndicators.hasBaseResources ? 20 : 0) +
      (resourceBalances["dysprosium"] && safeBigIntConversion(resourceBalances["dysprosium"]) > 0n
        ? 30
        : 0) +
      (resourceBalances["yttrium"] && safeBigIntConversion(resourceBalances["yttrium"]) > 0n
        ? 50
        : 0);

    metrics.compositeMetrics.verticalIntegration = Math.max(
      grapheneIntegration,
      yttriumIntegration
    );
    ...
    const resourcePathFocus =
      resourceAnalysis && !resourceAnalysis.error
        ? ((resourceAnalysis.pathTotals?.graphenePath?.percentage || 0) -
            (resourceAnalysis.pathTotals?.yttriumPath?.percentage || 0)) *
          2
        : 0;

    const liquidityPathFocus =
      liquidityAnalysis && !liquidityAnalysis.error && liquidityAnalysis.hasPositions
        ? liquidityAnalysis.strategyIndicators?.pathFocus || 0
        : 0;

    const stakingPathFocus =
      stakingAnalysis && !stakingAnalysis.error && stakingAnalysis.hasPositions
        ? stakingAnalysis.strategyIndicators?.pathFocus || 0
        : 0;
    ...
    // Resource diversity
    if (resourceAnalysis && !resourceAnalysis.error) {
      const resourceTypes = Object.keys(resourceAnalysis.resources || {}).filter(
        (r) => (resourceAnalysis.resources[r]?.percentage || 0) > 5
      ).length;
    ...
    if (liquidityAnalysis && !liquidityAnalysis.error && liquidityAnalysis.hasPositions) {
      diversityScore += Math.min(
        35,
        liquidityAnalysis.strategyIndicators?.resourceDiversification || 0
      );
    }
    ...
    if (stakingAnalysis && !stakingAnalysis.error && stakingAnalysis.hasPositions) {
      diversityScore += Math.min(
        35,
        stakingAnalysis.strategyIndicators?.yieldFarmingIntensity || 0
      );
    }
...
export function suggestCounterStrategies(
  ...
  try {
    ...
    const liquidityAnalysis = analyzeLiquidityStrategy(liquidityPositions);
    const stakingAnalysis = analyzeStakingStrategy(stakePositions);
    const overallStrategy = determineOverallStrategy(
      resourceBalances,
      liquidityPositions,
      stakePositions,
      he3Balance
    ...
    const analysis = {
      competitorProfile: {
        ...
        dominantPath:
          pathAnalysis?.overall?.pathDominance > 0
            ? "graphene"
            : pathAnalysis?.overall?.pathDominance < 0
              ? "yttrium"
              : "balanced",
    ...
    if (resourceAnalysis && !resourceAnalysis.error) {
      ...
      if (dominantResource && dominantResource.percentage > 50) {
        ...
        if (["carbon", "graphite", "graphene"].includes(dominantResource.name)) {
          analysis.competitiveOpportunities.resourceGaps.push(
            "Opportunity in yttrium path resources"
          );
          analysis.counterStrategyFactors.resourceCompetition.targetResources.push(
            "neodymium",
            "dysprosium",
            "yttrium"
          );
          ...
          analysis.competitiveOpportunities.resourceGaps.push(
            "Opportunity in graphene path resources"
          );
          analysis.counterStrategyFactors.resourceCompetition.targetResources.push(
            "carbon",
            "graphite",
        ...
        analysis.competitiveOpportunities.resourceGaps.push(
          "Opportunity to specialize for efficiency"
        );
    ...
    if (liquidityAnalysis && !liquidityAnalysis.error) {
      if (!liquidityAnalysis.hasPositions) {
        ...
        if (resourceTypes.advanced.percentage > 60) {
          ...
          analysis.counterStrategyFactors.liquidityStrategy.targetPairs.push(
            "carbon",
            "neodymium",
            "graphite",
            "dysprosium"
    ...
    analysis.counterStrategyFactors.he3Strategy.he3MiningEfficiency = Math.min(
      100,
      efficiencyScore
    );
    ...
    analysis.counterStrategyFactors.liquidityStrategy.shouldProvideLiquidity =
      !liquidityAnalysis?.hasPositions ||
      liquidityAnalysis.strategyIndicators?.resourceDiversification < 40;

    analysis.counterStrategyFactors.liquidityStrategy.shouldUndercut =
      liquidityAnalysis?.hasPositions &&
      liquidityAnalysis.strategyIndicators?.resourceDiversification > 70;

    analysis.counterStrategyFactors.stakingStrategy.shouldStake =
      !stakingAnalysis?.hasPositions ||
      stakingAnalysis.strategyIndicators?.yieldFarmingIntensity < 40;
    ...
    analysis.counterStrategyFactors.he3Strategy.shouldFocusHe3Staking =
      stakingAnalysis?.hasPositions &&
      stakingAnalysis.strategyIndicators?.he3DirectStakingFocus < 30;
  ...
  try {
    ...
    for (const [agentId, agentAddress] of Object.entries(agentAddresses)) {
      ...
      try {
        ...
        const liquidityPositionsResult = await executeQuery<LiquidityPositionsResponse>(
        ...
        const processedLiquidityPositions = {
          liquidityPosition:
            liquidityPositionsResult?.liquidityPosition?.map((pos) => ({
              ...pos,
              liquidity:
                typeof pos.liquidity === "object"
        ...
        const stakePositionsResult = await executeQuery<StakePositionsResponse>(
        ...
        const processedStakePositions = {
          agentStake:
            stakePositionsResult?.agentStake?.map((stake) => ({
              ...stake,
              stakedAmount:
                typeof stake.stakedAmount === "object"
                  ? (Object.values(stake.stakedAmount)[0] || "0").toString()
                  : (stake.stakedAmount || "0").toString(),
              rewards:
                typeof stake.rewards === "object"
 ...
 */
export async function rankAgentsByHe3(): Promise<
  Array<{ agentId: string; address: string; he3Balance: string }>
> {
  try {
    // Get all agent addresses
    const agentAddresses = getCategoryAddresses("agents");
 ...
 */
export async function getAgentResourceBalances(
  agentAddress: string
 ...
 */
export async function compareAgentPositions(
    ...
    const strategyArchetypes = [
      {
        ...
        applicability:
          competitorAnalysis.competitorProfile.diversificationLevel > 60 ? "High" : "Medium",
        creativityPrompts: [
      ...
      },
      {
        ...
        applicability: !competitorAnalysis.competitorProfile.liquidityActivity
          ? "High"
          : competitorAnalysis.strategicVulnerabilities.liquidityVulnerabilities.length > 0
            ? "Medium"
            : "Low",
        creativityPrompts: [
      ...
      },
      {
        ...
        applicability:
          Math.abs(competitorAnalysis.competitorProfile.pathDominanceScore) > 60
            ? "High"
            : "Medium",
        creativityPrompts: [
      ...
      },
      {
        ...
        applicability:
          competitorAnalysis.competitorProfile.verticalIntegrationLevel < 50 ? "High" : "Low",
        creativityPrompts: [
