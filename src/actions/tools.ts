...
export const toolActions = [
  action({
    ...
    instructions:
      "Use this action when an agent needs to understand the game rules, contract addresses, and protocol parameters",
    schema: z.object({
  ...
  action({
    ...
    instructions:
      "Use this action when an agent needs to convert blockchain uint256 values to readable decimal numbers",
    schema: z.object({
  ...
  action({
    ...
    instructions:
      "Use this action when an agent needs to check their token balances or monitor another agent's resources",
    schema: z.object({
      agentId: z
        .string()
        .describe("agentID to query (defaults to current agent if not provided)")
        .optional(),
    }),
    async handler(args, ctx, agent) {
      try {
  ...
  action({
    ...
    instructions:
      "Use this action when an agent needs to analyze differences in strategy with another agent to learn or develop counter-strategies",
    schema: z.object({
  ...
  action({
    ...
    instructions:
      "Use this action when an agent needs to understand the competitive landscape and identify leading agents",
    schema: z.object({
      message: z.string().describe("Not used - can be ignored").default("None"),
    }),
    async handler(args, ctx, agent) {
